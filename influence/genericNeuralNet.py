...
class GenericNeuralNet(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.learning_rate = tf.Variable(
            self.initial_learning_rate, name='learning_rate', trainable=False)
        ...
        self.train_sgd_op = self.get_train_sgd_op(
            self.total_loss, self.global_step, self.learning_rate)
        self.accuracy_op = self.get_accuracy_op(
        ...
        self.v_placeholder = [tf.placeholder(
            tf.float32, shape=a.get_shape()) for a in self.params]
        self.u_placeholder = [tf.placeholder(
            tf.float32, shape=a.get_shape()) for a in self.params]
        ...
        # Because tf.gradients auto accumulates, we probably don't
        ...
        self.checkpoint_file = os.path.join(
            self.train_dir, "%s-checkpoint" % self.model_name)
    ...
    def fill_feed_dict_with_some_ex(self, data_set, target_indices):
        input_feed = data_set.x[target_indices,
        ...
        for i in xrange(num_iter):
            ...
            if len(ret) == 0:
                ...
                for counter, b in enumerate(ret_temp):
                    if isinstance(b, list):
                        ret[counter] = [a + (c / float(num_iter))
                                        for (a, c) in zip(ret[counter], b)]
                    else:
    ...
    def print_model_eval(self):
        ...
        if self.mini_batch == True:
            (
                ...
                loss_val,
                train_acc_val,
            ) = self.minibatch_mean_eval(
                [self.grad_total_loss_op, self.loss_no_reg,
                    self.total_loss, self.accuracy_op],
                self.data_sets.train
            )
        ...
        print('Norm of the mean of gradients: %s' %
              np.linalg.norm(np.concatenate(grad_loss_val)))
        print('Norm of the params: %s' %
              np.linalg.norm(np.concatenate(params_val)))
    ...
    def update_learning_rate(self, step):
        ...
        self.sess.run(
            ...
            feed_dict={self.learning_rate_placeholder:
                       multiplier * self.initial_learning_rate})
    ...
    def train(self, num_steps,
        ...
        for step in xrange(num_steps):
            ...
            if step < iter_to_switch_to_batch:
                feed_dict = self.fill_feed_dict_with_batch(
                    self.data_sets.train)
                _, loss_val = sess.run(
                    [self.train_op, self.total_loss], feed_dict=feed_dict)
                ...
                feed_dict = self.all_train_feed_dict
                _, loss_val = sess.run(
                    [self.train_sgd_op, self.total_loss], feed_dict=feed_dict)
    ...
    def loss(self, logits, labels):
        ...
        cross_entropy = - \
            tf.reduce_sum(tf.multiply(
                labels, tf.nn.log_softmax(logits)), reduction_indices=1)
    ...
    def adversarial_loss(self, logits, labels):
        ...
        wrong_labels_bool = tf.reshape(
            tf.cast(wrong_labels, tf.bool), [-1, self.num_classes])

        wrong_logits = tf.reshape(tf.boolean_mask(
            logits, wrong_labels_bool), [-1, self.num_classes - 1])
    ...
    def get_inverse_hvp_lissa(self, v,
        ...
        for i in range(num_samples):
            ...
            for j in range(recursion_depth):
                ...
                #   samples[j])
                feed_dict = self.fill_feed_dict_with_batch(
                    self.data_sets.train, batch_size=batch_size)

                feed_dict = self.update_feed_dict_with_v_placeholder(
                    feed_dict, cur_estimate)
                hessian_vector_val = self.sess.run(
                    self.hessian_vector, feed_dict=feed_dict)
            ...
            if inverse_hvp is None:
                ...
                inverse_hvp = [
                    a + b/scale for (a, b) in zip(inverse_hvp, cur_estimate)]
    ...
    def minibatch_hessian_vector_val(self, v):
        ...
        for i in xrange(num_iter):
            ...
            hessian_vector_val_temp = self.sess.run(
                self.hessian_vector, feed_dict=feed_dict)
            if hessian_vector_val is None:
                hessian_vector_val = [b / float(num_iter)
                                      for b in hessian_vector_val_temp]
            else:
                hessian_vector_val = [
                    a + (b / float(num_iter)) for (a, b) in zip(hessian_vector_val, hessian_vector_val_temp)]

        hessian_vector_val = [a + self.damping *
                              b for (a, b) in zip(hessian_vector_val, v)]
    ...
    def get_cg_callback(self, v, verbose):
        ...
        def cg_callback(x):
            ...
            predicted_loss_diff = np.dot(np.concatenate(v), np.concatenate(
    ...
    def get_test_grad_loss_no_reg_val(self, test_indices, batch_size=100, loss_type='normal_loss'):
        ...
        if test_indices is not None:
            ...
            for i in range(num_iter):
                ...
                test_feed_dict = self.fill_feed_dict_with_some_ex(
                    self.data_sets.test, test_indices[start:end])
                ...
                if test_grad_loss_no_reg_val is None:
                    ...
                    test_grad_loss_no_reg_val = [
                        a + b * (end-start) for (a, b) in zip(test_grad_loss_no_reg_val, temp)]

            test_grad_loss_no_reg_val = [
                a/len(test_indices) for a in test_grad_loss_no_reg_val]

        else:
            test_grad_loss_no_reg_val = self.minibatch_mean_eval(
                [op], self.data_sets.test)[0]
    ...
    def get_influence_on_test_loss(self, test_indices, train_idx,
        ...
        test_grad_loss_no_reg_val = self.get_test_grad_loss_no_reg_val(
            test_indices, loss_type=loss_type)

        print('Norm of test gradient: %s' % np.linalg.norm(
            np.concatenate(test_grad_loss_no_reg_val)))
        ...
        if train_idx is None:
            ...
            for counter in np.arange(num_to_remove):
                ...
                train_grad_loss_val = self.sess.run(
                    self.grad_total_loss_op, feed_dict=single_train_feed_dict)
                predicted_loss_diffs[counter] = np.dot(np.concatenate(
                    inverse_hvp), np.concatenate(train_grad_loss_val)) / self.num_train_examples
    ...
    def find_eigvals_of_hessian(self, num_iter=100, num_prints=10):
        ...
        grad_loss_val = self.sess.run(
            self.grad_total_loss_op, feed_dict=feed_dict)
        initial_v = []
        ...
        for i in range(num_iter):
            ...
            hessian_vector_val = self.minibatch_hessian_vector_val(
                cur_estimate)
            new_cur_estimate = [
                a - largest_eig * b for (a, b) in zip(hessian_vector_val, cur_estimate)]

            if i % print_iterations == 0:
                print(-norm_val + largest_eig)
                dotp = np.dot(np.concatenate(new_cur_estimate),
                              np.concatenate(cur_estimate))
                print("dot: %s" % dotp)
    ...
    def get_grad_of_influence_wrt_input(self, train_indices, test_indices,
        ...
        for counter, train_idx in enumerate(train_indices):
            ...
            self.update_feed_dict_with_v_placeholder(
                grad_influence_feed_dict, inverse_hvp)

            # Run the grad op with the feed dict
            current_grad_influence_wrt_input_val = self.sess.run(
                self.grad_influence_wrt_input_op, feed_dict=grad_influence_feed_dict)[0][0, :]
            ...
            grad_influence_wrt_input_val[counter,
    ...
    def update_train_x(self, new_train_x):
        ...
        self.data_sets = base.Datasets(
            train=new_train, validation=self.data_sets.validation, test=self.data_sets.test)
        self.all_train_feed_dict = self.fill_feed_dict_with_all_ex(
            self.data_sets.train)
        self.reset_datasets()
    ...
    def update_test_x_y(self, new_test_x, new_test_y):
        ...
        self.data_sets = base.Datasets(
            train=self.data_sets.train, validation=self.data_sets.validation, test=new_test)
        self.all_test_feed_dict = self.fill_feed_dict_with_all_ex(
            self.data_sets.test)
