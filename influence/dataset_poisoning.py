...
def select_examples_to_attack(model, num_to_poison, grad_influence_wrt_input_val, step_size):
    ...
    # First index is the most effective
    indices_to_poison = np.argsort(pred_diff)[-1:-num_to_poison-1:-1]
...
def generate_inception_features(model, poisoned_X_train_subset, labels_subset, batch_size=None):
    ...
    poisoned_data_sets = base.Datasets(
        train=poisoned_train, validation=None, test=None)
    ...
    for i in xrange(num_iter):
        feed_dict = model.fill_feed_dict_with_batch(
            poisoned_data_sets.train, batch_size=batch_size)
        inception_features_val_temp = model.sess.run(
            model.inception_features, feed_dict=feed_dict)
...
def iterative_attack(top_model, full_model, top_graph, full_graph, project_fn, test_indices, test_description=None,
    ...
    if early_stop is not None:
        assert len(
            test_indices) == 1, 'Early stopping only supported for attacks on a single test index.'
    ...
    for attack_iter in range(num_iter):
        ...
        copyfile(
            'output/%s-cg-%s-test-%s.npz' % (top_model_name,
                                             loss_type, test_description),
        ...
        with full_graph.as_default():
            ...
            inception_X_train_subset = generate_inception_features(
                full_model, poisoned_X_train_subset, labels_subset)
        ...
        with top_graph.as_default():
            ...
            weight_path = 'output/inception_weights_%s_attack_%s_testidx-%s.npy' % (
                top_model_name, loss_type, test_description)
            np.save(weight_path, weights)
        with full_graph.as_default():
            full_model.load_weights_from_disk(
                weight_path, do_save=False, do_check=False)
