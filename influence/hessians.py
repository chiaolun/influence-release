...
def hessians(ys, xs, name="hessians", colocate_gradients_with_ops=False,
    ...
    for i, x in enumerate(xs):
        ...
        with ops.name_scope(name + '_first_derivative'):
            # Compute the partial derivatives of the input with
            # respect to all elements of `x`
            _gradients = tf.gradients(ys, x, **kwargs)[0]
            # Unpack the gradients into a list so we can take
            # derivatives with respect to each element
            _gradients = array_ops.unpack(_gradients)
        with ops.name_scope(name + '_second_derivative'):
            # Compute the partial derivatives with respect to each
            # element of the list
            _hess = [tf.gradients(_gradient, x, **kwargs)[0]
